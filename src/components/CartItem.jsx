    ...
    <div>
      <div className='flex items-center justify-between p-2'>
        ...
        <div className='w-1/6 flex gap-2'>
          <button className='border px-1' onClick={() => dispatch(cartActions.removeItemFromCart({ id }))}><FontAwesomeIcon icon={faMinus} /></button>
          <p className='w-8 text-center'>{quantity}</p>
          <button className='border px-1' onClick={() => dispatch(cartActions.addItemToCart({ id, title, price }))}><FontAwesomeIcon icon={faPlus} /></button>
        </div>
